...
Archetype 5, per this project's own archetype-to-paper mapping (`data/basis_2025_26/archetype_labels.csv`, built in `src/step1_archetypes_model.py`'s `label_archetypes_vs_paper`), corresponds to the paper's own **"Shooting Specialist"** archetype — a strong match: MPJ's top feature signature (off-screen/handoff usage, low self-creation, high 3PA share, low STL%/TRB%) lines up closely with the paper's own description ("movement or spot-up shooter... low self-creation, low 2FGA%"). MPJ's exemplar teammate-in-feature-space is Tim Hardaway Jr.
